 ...
 */
	...
	function getSmiles() {
		...
		var field = document.getElementById("mol");
		field.value = document.JME.smiles();
	}
        function saveMoleculeSearch() {
		var field = document.getElementById("id_obrazek");
		field.value = document.JME.smiles();
	}
    ...
    function checkAll(ele) {
        var checkboxes = document.getElementsByName('mlk_id');
        if (ele.checked) {
    ...
    function hidEl(ele) {
        var i = 'id_'+ ele.id;
        var hid = document.getElementById(i);
        if (hid.disabled) {
            if (hid.id == 'id_mol_weight') {
                var hid2 = document.getElementById('id_opMW');
                hid.disabled = false;
                hid.className = 'w3-input w3-border w3-light-grey';
                hid2.disabled = false;
                hid2.className = 'w3-input w3-border w3-light-grey';
                hid.required = true;
            }else if (hid.id == 'id_mol_stock'){
                var hid2 = document.getElementById('id_opSt');
                hid.disabled = false;
                hid.className = 'w3-input w3-border w3-light-grey';
                hid2.disabled = false;
                hid2.className = 'w3-input w3-border w3-light-grey';
            ...
            if (hid.id == 'id_mol_weight') {
                ...
                hid2.disabled = true;
                hid2.className = 'w3-input w3-border w3-light-grey w3-hide';
                hid.required = false;
            }else if (hid.id == 'id_mol_stock'){
                var hid2 = document.getElementById('id_opSt');
                hid.disabled = true;
